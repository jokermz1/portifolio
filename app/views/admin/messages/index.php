...
<div class="page-header">
    <div>
        ...
        <span style="font-size:12px; color:var(--text-faint);">
            <?= count($messages) ?> mensage<?= count($messages) !== 1 ? 'ns' : 'm' ?>
            <?php if (count($unread) > 0): ?>
            · <span style="color:var(--accent);"><?= count($unread) ?> não lida<?= count($unread) > 1 ? 's' : '' ?></span>
            <?php endif; ?>
        </span>
    </div>
</div>
                ...
                <tbody>
                    ...
                    <tr class="<?= !$m['is_read'] ? 'table-active' : '' ?>">
                        ...
                        <td style="color:<?= !$m['is_read'] ? 'var(--text-primary)' : 'var(--text-muted)' ?>">
                            <?= htmlspecialchars(mb_strimwidth($m['subject'] ?? '(sem assunto)', 0, 55, '…')) ?>
                        </td>
                        ...
                        <td>
                            ...
                            <form method="POST" action="<?= BASE_URL ?>/admin/messages/<?= $m['id'] ?>/delete"
                                  class="d-inline" onsubmit="return confirm('Apagar esta mensagem?')">
